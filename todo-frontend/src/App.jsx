...
function App() {
  ...
  const manejarCompletarTarea = (id) => {
    fetch(`http://localhost:8080/tareas/${id}/completar`, {
      method: 'PUT'
    })
    .then(response => {
      if (response.ok) {
    ...
    <div style={{ maxWidth: '700px', margin: '40px auto', padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {tareas.length === 0 && <p style={{ color: '#888', fontStyle: 'italic', marginTop: '20px' }}>No hay tareas registradas en el Gestor.</p>}
        </div>
      )}
    </div>
  )
